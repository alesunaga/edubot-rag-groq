...
# --- PAGE CONFIGURATION ---
st.set_page_config(
    page_title="EduBot RAG - Groq Edition",
    page_icon="⚡",
    layout="wide"
)
...
def get_groq_models(api_key):
    try:
        ...
        # Filter to show only text models (Llama/Mixtral) and sort
...
def get_groq_response(api_key, model_name, prompt):
    try:
        client = Groq(api_key=api_key)
...
with st.sidebar:
    ...
    # Password Field (Groq Only)
    user_api_key = st.text_input("Groq API Key (gsk_...)", type="password", value=st.session_state.api_key)
    
    if user_api_key:
        st.session_state.api_key = user_api_key
        # Load models dynamically if needed
    ...
    if st.session_state.available_models:
        ...
        st.caption("Suggestion: Llama-3.3-70b (Smarter) or Llama-3.1-8b (Faster)")
...
if role_view == "Teacher":
    ...
    with tab1:
        ...
        st.session_state.rag_mode = st.radio("Reading Method:", 
            ...
            help="Simple: Pastes the whole text. Advanced: Uses intelligent search (Local Embeddings).")
        ...
        role = col1.selectbox("Bot Role", list(ROLES_PROMPTS.keys()))
        lang = col2.selectbox("Response Language", ["English", "Portuguese", "Spanish", "German"])
...
# --- STUDENT INTERFACE ---
    ...
    if prompt := st.chat_input("Type your question..."):
        if not selected_model:
            st.error("⚠️ Teacher: Please configure the Groq Key in the sidebar first.")
            st.stop()
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                response = get_groq_response(st.session_state.api_key, selected_model, full_prompt)
